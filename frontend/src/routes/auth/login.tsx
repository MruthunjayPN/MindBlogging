...
export default function LoginPage() {
  ...
  const { register, handleSubmit, formState: { errors } } = useForm({
    resolver: zodResolver(userAuthSchema.signin)
  });

  const onSubmit = async (data: z.infer<typeof userAuthSchema.signin>) => {
    try {
  ...
  return (
    <div className="min-h-screen grid grid-cols-2">
      ...
      <div className="flex flex-col items-center justify-center p-8 bg-night">
        <Logo variant="auth" />
        <div className="w-full mt-6 max-w-md relative overflow-hidden">
          <ShineBorder className="absolute inset-0" />
          <Card className='bg-platinum'>
            <CardHeader>
              <CardTitle className="text-3xl flex justify-center dark:text-eblack">Login</CardTitle>
            </CardHeader>
            <CardContent>
              ...
              <div className="text-center mt-4">
                <p className="text-sm text-space">
                  ...
                  <a href="/register" className=" text-eblack hover:underline">
                    Signup
                  </a>
                </p>
              </div>
      ...
      <div className="bg-zinc-900 flex flex-col justify-center overflow-hidden">
